...
//[Nesne Havuzu] Yöntemiyle Platformları listeye dahil etme ve taşıma.
public class PlatformPool : MonoBehaviour
{
    ...
    void PlatformUret()
    {
        ...
        platforms.Add(ilkPlatform);//İlk paltformu listeye ekledik.
        SonrakiPlatformPozisyon();
        ilkPlatform.GetComponent<Platform>().Hareket = true;//İlk platformu hareketsiz yaptık.

        for (int i = 0; i < 8; i++)//8 platform üretileek çünkü ilki zaten üretildi ve bir tanesi de ölümcül platform olacak.
        {
    ...
    void PlatformYerlestir()
    {
        for (int i = 0; i < 5; i++)
        {
            //Platform sırasını listede değiştirmek:
            GameObject temp;
            temp = platforms[i + 5];
            platforms[i + 5] = platforms[i];
            platforms[i] = temp;

            //Düzenlenen listeye göre en altta kalan platformların sahnedeki yerini değiştirmek:
